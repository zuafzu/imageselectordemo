...
class MainActivity : AppCompatActivity() {
    ...
    private fun initRcv() {
        val config = PhotoConfigure().setType(PhotoConfigure.EditImg).setColnum(3)
        recyclerView.show(config)
...
//        }).show(config)
        ...
        button.setOnClickListener {
            // 获取压缩图片(需要在回调方法setOnCyfThumbnailsListener中获取)
            // recyclerView.getSelectThumbnailsList()
            // 实原始数据
            Log.e("cyf111", "222 : " + recyclerView.selectList)
        }
    }
