package com.cyf.cyfimageselector.pinchimage;
...
public class PinchImageView extends android.support.v7.widget.AppCompatImageView {
    ...
    public Matrix getInnerMatrix(Matrix matrix) {
        ...
        if (isReady()) {
            ...
            tempSrc = MathUtils.rectFTake(0, 0, getDrawable().getIntrinsicWidth(), getDrawable().getIntrinsicHeight());
            tempDst = MathUtils.rectFTake(0, 0, getWidth(), getHeight());
...
//            } else {
//                tempSrc = MathUtils.rectFTake(0, 0, getDrawable().getIntrinsicWidth(), getDrawable().getIntrinsicHeight());
//                tempDst = MathUtils.rectFTake(0, 0, getWidth(), getDrawable().getIntrinsicHeight());
//            }
    ...
    public boolean canScrollHorizontally(int direction) {
        ...
        if (direction > 0) {
            ...
            return bound.right > (getWidth() + 0.0001f);
        } else {
            // -------------cyf 二次修改（有些图片不到0，但无限接近0）--------------
            // return bound.left < 0;
            return bound.left < -3.1981872E-6;
        }
    }
